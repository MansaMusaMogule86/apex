        ...
        return (
          <li key={item.href}>
            <Link
              ...
              onClick={onNavigate}
              className={cn(
                "group relative flex items-center gap-3 rounded-[2px] px-3 py-2.5 transition-colors duration-300",
                active ? "bg-gold/[0.08] text-warm-white" : "text-titanium hover:bg-white/[0.04] hover:text-warm-white",
              )}
              data-cursor="interactive"
            >
              {active ? <span className="absolute left-0 top-2 bottom-2 w-px bg-gold" /> : null}
              <Icon className={cn("h-4 w-4 shrink-0", active ? "text-gold" : "text-mist")} strokeWidth={1.5} />
              {!collapsed ? (
  ...
  return (
    <>
      <motion.aside
        ...
        className="hidden xl:flex xl:h-[calc(100vh-24px)] xl:flex-col xl:overflow-hidden xl:rounded-[2px] xl:border xl:border-gold/15 xl:bg-[var(--cc-surface)] xl:backdrop-blur-2xl"
      >
        <div className="flex h-16 items-center border-b border-gold/10 px-4">
          <Link href="/dashboard" className="flex items-center gap-3 overflow-hidden" data-cursor="interactive">
            <span className="flex h-8 w-8 items-center justify-center rounded-[2px] border border-gold/25 bg-void font-display text-sm text-gold">
              A
            </span>
            {!sidebarCollapsed ? (
        ...
        {mobileSidebarOpen ? (
          ...
          >
            ...
            <motion.aside
              ...
              exit={{ x: -320 }}
              transition={{ duration: 0.35, ease: COMMAND_EASE }}
              className="relative h-full w-[88%] max-w-[320px] border-r border-gold/20 bg-obsidian p-4"
            >
