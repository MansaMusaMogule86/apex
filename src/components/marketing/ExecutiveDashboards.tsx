...
export default function ExecutiveDashboards() {
  return (
    <section className="border-y border-white/10 bg-obsidian/85 py-24 md:py-32" id="executive-dashboards">
      <div className="mx-auto max-w-335 px-4 md:px-8">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-5">
          <div>
            <p className="mb-4 font-mono text-[10px] uppercase tracking-[0.4em] text-signal-blue">Executive Dashboards</p>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.35fr_0.9fr]">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {PANELS.map(([label, value, delta]) => (
                <motion.div key={label} whileHover={{ y: -4 }} className="rounded-[2px] border border-white/10 bg-white/[0.03] p-4" data-cursor="interactive">
                  ...
                  <p className="mt-2 font-display text-4xl text-platinum">{value}</p>
                  <p className="mt-2 font-mono text-[10px] uppercase tracking-[0.2em] text-signal-blue">{delta}</p>
                </motion.div>
              ))}
            </div>

            <div className="mt-4 rounded-[2px] border border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(110,140,255,0.16),transparent_56%),#0a0a0f] p-5">
              <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-signal-blue">Luxury Audience Heatmap</p>
              <div className="mt-4 grid grid-cols-8 gap-1.5">
                {Array.from({ length: 64 }).map((_, index) => (
                  <span
                    ...
                    style={{
                      backgroundColor: `rgba(110,140,255,${Math.max(0.08, (index % 9) / 9)})`,
                    }}
                  />
                ))}
              </div>
          ...
          <div className="rounded-[2px] border border-white/12 bg-gradient-to-b from-signal-blue/[0.09] to-transparent p-5 md:p-6">
            <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-signal-blue">Real-time Intelligence Feed</p>
            <div className="mt-4 space-y-3">
              {FEED.map((line) => (
                <div key={line} className="rounded-[2px] border border-white/10 bg-black/30 p-3">
                  <p className="text-sm leading-6 text-platinum/90">{line}</p>
                </div>
              ))}
            </div>
          </div>
